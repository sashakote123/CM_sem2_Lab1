...
def f_var3(x):
    return np.sin(x + 1) / (x + 1) + np.cos(10 * x)


def d_f_var3(x):
    return (np.cos(x + 1) * (x + 1) - np.sin(x + 1)) / (x + 1) ** 2 - 10 * np.sin(10 * x)


def d2_f_var3(x):
    return ((x + 1) * (np.cos(x + 1) * (x + 1) - np.sin(x + 1)) - np.cos(x + 1) * (x + 1) + 2 * np.sin(x + 1)) / (
            x + 1) ** 3 - 100 * np.cos(10 * x)
...
def setSolveVector(N, a, b):
    ...
    Solve.append(0)
    for i in range(N - 2):
        Solve.append(6 * ((f_var3(x + h) - f_var3(x)) / h - (f_var3(x) - f_var3(x - h)) / h))
        x += h
