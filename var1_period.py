...
def f_var1_period(x):
    return np.sqrt(x ** 2 - 1) / x + np.cos(10*x)


def d2_f_var1_period(x):
    return (2 - 3 * x ** 2) / (np.sqrt(x ** 2 - 1) * x ** 3 * (x ** 2 - 1)) - 100*np.cos(10*x)
...
def setSolveVector(N, a, b):
    ...
    for i in range(N - 2):
        Solve.append(
            6 * ((f_var1_period(x + h) - f_var1_period(x)) / h - (f_var1_period(x) - f_var1_period(x - h)) / h))
        x += h
...
def GetCoefficients(n, a, b):
    ...
    x0 = a
    for i in range(n + 1):
        List_a.append(f_var1_period(x0))
        x0 += h1
...
def createSpline(n, a, b):
    ...
    for i in range(1, n + 1):
        ...
        List_F.append(f_var1_period(List_x[i - 1]))

        List_dS.append(
            List_b[i - 1] + List_c[i] * (List_x[i - 1] - x0) + (List_d[i - 1] / 2) * (List_x[i - 1] - x0) ** 2)
        List_d2S.append(List_c[i] + List_d[i - 1] * (List_x[i - 1] - x0))

        List_dF.append(1 / ((List_x[i - 1] ** 2 - 1) ** (1 / 2) * List_x[i - 1] ** 2) - 10*np.sin(10*List_x[i - 1]))
        List_d2F.append(d2_f_var1_period(List_x[i - 1]))
        x0 += h1
