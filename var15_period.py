...
def f_var15(x):
    return np.cos(x ** 2 / 4)+np.cos(10*x)


def d_f_var15(x):
    return -np.sin(x ** 2 / 4) * x / 2- 10*np.sin(10*x)


def d2_f_var15(x):
    return -np.cos(x ** 2 / 4) * x ** 2 / 4 - np.sin(x ** 2 / 4) / 2 - 100*np.cos(10*x)
...
def setSolveVector(N, a, b):
    ...
    Solve.append(0)
    for i in range(N - 2):
        Solve.append(6 * ((f_var15(x + h) - f_var15(x)) / h - (f_var15(x) - f_var15(x - h)) / h))
        x += h
...
def getdata_var15_period(N, a, b):
