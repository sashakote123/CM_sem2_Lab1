...
def d2_f_var1(x):
    return (2 - 3 * x ** 2) / (math.sqrt(x ** 2 - 1) * x ** 3 * (x ** 2 - 1))
...
def setSolveVector(N, a, b):
    ...
    Solve.append(0)
    for i in range(N - 2):
        Solve.append(6 * ((f_var1(x + h) - f_var1(x)) / h - (f_var1(x) - f_var1(x - h)) / h))
        x += h
...
def createSpline(n, a, b):
    ...
    for i in range(1, n + 1):
        ...
        List_dF.append(1 / ((List_x[i - 1] ** 2 - 1) ** (1 / 2) * List_x[i - 1] ** 2))
        List_d2F.append((2 - 3 * List_x[i - 1] ** 2) / (
                (List_x[i - 1] ** 2 - 1) ** (1 / 2) * List_x[i - 1] ** 3 * (List_x[i - 1] ** 2 - 1)))
        x0 += h1
